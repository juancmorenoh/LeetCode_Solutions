...
Description: You are given a 0-indexed string s typed by a user. 
...
that is if a user typed the letter 'a' and then the letter 'A' 
...
def countKeyChanges(s):
    ...
    for i in range(len(s)-1): #loop throug string and avoid out of range error
        if s[i].lower() != s[i+1].lower(): #ensure case-insensitive by lower casing each char
            counter += 1 
    return counter
